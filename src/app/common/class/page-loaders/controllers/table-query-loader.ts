...
export class TableQueryLoader<T> extends LoadingController<T> implements LoadingHandler<T> {
    ...
    constructor(loader: Pageable<T>, router: Router, route: ActivatedRoute, target: string[]) {
        ...
        this.subscriptions = this.loader.liveUpdateProvides().map((observable, index) => observable.subscribe(provider => {
                ...
                this.elements = updateListResolver(this.elements, this.loader.liveUpdateIdentificationFields()[index], provider, this.loader.liveUpdateIdentificationFields()[index] !== "beginningId");
            }
        ))
    }
    ...
    public limitUpdate(event: LazyLoadEvent) {
        ...
        if (this.isInit) this.router.navigate(this.target, {
            queryParams: {
