...
export class MeasurePageComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		console.log("MeasurePageComponent.ngOnInit");
		...
		this.subscribes.push(
			this.service.downloadTest.getUpdateTest().subscribe((result) => {
				this.chart[0] = result.chartData;
				this.chart = this.chart.slice();
				this.testType = "download";
				this.indicatorSpeed = result.speed;
			}),
		);
		this.subscribes.push(
			this.service.uploadTest.getUpdateTest().subscribe((result) => {
				this.chart[1] = result.chartData;
				this.chart = this.chart.slice();
	...
	ngOnDestroy(): void {
		console.log("MeasurePageComponent.ngOnDestroy");
